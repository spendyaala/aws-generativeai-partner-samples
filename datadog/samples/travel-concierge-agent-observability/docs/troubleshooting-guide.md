...
│   │       └── HTTP GET serpapi.com                 █ ERROR                   0.1s  ← ROOT CAUSE
│   └── LLM: bedrock.converse (error handling)      ██                        0.5s
```

The error propagates upward: `HTTP GET serpapi.com` → `travel-mcp-server` → `travel_hotel_search` → `travel_assistant` → `supervisor-agent`.
...
- **Error Message:** `401 Unauthorized — SerpAPI key invalid or expired`
